...
function ENT:Use(activator,caller)
	...
	if activator.maxsDrugs and activator.maxsDrugs >= GAMEMODE.Config.maxsdrugs then
		GAMEMODE:Notify(activator, 1, 3, "You can't make anymore stable drugs as the limit is reached.")
		timer.Simple(0.5, function() self.CanUse = true end)
	else

		local productioncost = math.random(200,300)
		...
		GAMEMODE:Notify(activator, 0, 4, "You have made stable drugs! production cost: " .. CUR .. tostring(productioncost).."!")
		self.sparking = true
		timer.Create(self:EntIndex() .. "stabledrug", 1, 1, function() self:createDrug() end)
	end
end
...
function ENT:createDrug()
	...
	drug:Setprice(300)
	drug:Spawn()
	if not userb.maxsDrugs then
		userb.maxsDrugs = 0
	end
	userb.maxsDrugs = userb.maxsDrugs + 1
	self.sparking = false
end
...
function ENT:OnRemove()
	...
	timer.Destroy(self:EntIndex() .. "stabledrug")
	self:Destruct()
end