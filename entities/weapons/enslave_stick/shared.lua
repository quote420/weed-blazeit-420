...
function SWEP:PrimaryAttack()
	...
	if IsValid(trace.Entity) and trace.Entity:IsPlayer() and not GAMEMODE.Config.slavery then
		GAMEMODE:Notify(self.Owner, 1, 5, "Slavery is not allowed here.")
		return
	end
	...
	local spc = DB.CountSlavePos()

	if not spc or spc == 0 then
		GAMEMODE:Notify(self.Owner, 1, 4, "You cannot enslave people since there are no slave shop positions set!")
	else
		-- Send NPCs to Jail
		if trace.Entity:IsNPC() then
			trace.Entity:SetPos(DB.RetrieveSlavePos())
		else
			if not trace.Entity.Babygod then
				trace.Entity:Arrest()
				GAMEMODE:Notify(trace.Entity, 0, 20, "You've been enslaved!" )
				...
				GAMEMODE:Notify(self.Owner, 1, 4, "You can't enslave players who are spawning.")
			end
		end
	end
end
