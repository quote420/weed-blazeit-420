...
if ( CLIENT ) then
	...
	SWEP.SlotPos			= 1
	SWEP.IconLetter			= "D"
	surface.CreateFont("CSKillIcons", {font = "csd", size = ScreenScale(40), weight = 500, antialias = true})
	surface.CreateFont("RCSSelectIcons", {font = "cs", size = ScreenScale(60), weight = 500, antialias = true})
...
function SWEP:PrimaryAttack()
	local deployDELAY = self.Weapon:GetNetworkedInt("deploydelay");
	if (deployDELAY > CurTime() || self.ShootafterTakeout > CurTime() || !self:CanPrimaryAttack()) then return end
...
function SWEP:CSShootBullet( dmmg, recoil, amount, spread )
	-- That was really dumb! Who put a font create here?
	self:ShootEffects()
	...
	rcsbul1.Callback	 = function ( attacker, btr, dmginfo ) --boom headshot
		if ( btr.HitGroup == HITGROUP_HEAD ) then
			if CLIENT then killicon.AddFont( self.NameOfSWEP, "CSKillIcons", self.IconLetter.."D", Color( 255, 80, 0, 255 ) ) end
		else
			if CLIENT then killicon.AddFont( self.NameOfSWEP, "CSKillIcons", self.IconLetter, Color( 255, 80, 0, 255 ) ) end
		end
	end 
	...
	if ( self.Weapon:GetNetworkedBool("Silenced") != true or self.IsAlwaysSilenced != true ) then --only does the muzzleflash if you are not silenced
	...
	if self.BurstType == "pistol" then --glock stuff
...
function SWEP:ShootEffects()
	if !self.Weapon:GetNetworkedBool("Silenced", true) then --no muzzleflash if its silenced
		self.Owner:MuzzleFlash() --crappy muzzle light
	end	
	self.Owner:SetAnimation( PLAYER_ATTACK1 ) --3rd Person Animation
